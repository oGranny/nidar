...
class GeoProviderProtocol(Protocol):  # pragma: no cover - typing helper
...
class MAVLinkGeoProvider(GeoProviderProtocol):
...
class MockGeoProvider(GeoProviderProtocol):
    """Simple telemetry generator used when pymavlink is unavailable."""

    def __init__(
        self,
        start_lat: float = 37.4221,
        start_lon: float = -122.0841,
        altitude_m: float = 45.0,
    ) -> None:
        self._lock = threading.Lock()
        self._base_lat = start_lat
        self._base_lon = start_lon
        self._base_alt = altitude_m
        self.lat = start_lat
        self.lon = start_lon
        self.alt = altitude_m
        self.ts = time.time()
    ...
    def _loop(self) -> None:
        while self._running:
            time.sleep(1.0)
            self._phase += 1.0
            lat_offset = 0.00015 * math.sin(self._phase / 20.0)
            lon_offset = 0.00015 * math.cos(self._phase / 25.0)
            alt_offset = 3.0 * math.sin(self._phase / 45.0)
            with self._lock:
                self.lat = self._base_lat + lat_offset
                self.lon = self._base_lon + lon_offset
                self.alt = self._base_alt + alt_offset
                self.heading = (self.heading + 8.0) % 360
                self.groundspeed = 5.0 + 1.2 * math.sin(self._phase / 10.0)
                self.ts = time.time()
...
def build_app_callback(
    mav_provider: GeoProviderProtocol,
