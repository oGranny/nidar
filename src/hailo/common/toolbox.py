...
RESOURCES_DOWNLOAD_DIR = Path(__file__).resolve().parent
GET_HEF_BASH_SCRIPT_PATH   = RESOURCES_DOWNLOAD_DIR.parent.parent / "resource_download" / "get_hef.sh"
GET_INPUT_BASH_SCRIPT_PATH = RESOURCES_DOWNLOAD_DIR.parent.parent / "resource_download" / "get_input.sh"
RESOLUTION_MAP = {
    "sd": (640, 480),
    "hd": (1280, 720),
    "fhd": (1920, 1080)
...
def init_input_source(input_path, batch_size, camera_resolution):
    ...
    Returns:
        Tuple[Optional[cv2.VideoCapture], Optional[List[np.ndarray]], Optional[Picamera2]]
    """
    cap = None
    images = None
    picam2 = None
    ...
    if input_path == "camera":
        ...
        width, height = RESOLUTION_MAP.get(camera_resolution, (1280, 720))
        
        # Try Raspberry Pi camera with picamera2 first
        try:
            ...
            picam2.start()
            logger.info(f"Opened Raspberry Pi camera via picamera2 at {width}x{height}")
        except (ImportError, Exception) as e:
            logger.info(f"picamera2 not available ({e}), trying standard V4L2 camera...")
            picam2 = None
            
            # Fallback to standard V4L2 camera
...
def preprocess(images: List[np.ndarray], cap: cv2.VideoCapture, framerate: float, batch_size: int,
    ...
    Args:
        ...
        picam2: Optional Picamera2 instance for Raspberry Pi camera.
    """
    preprocess_fn = preprocess_fn or default_preprocess

    if picam2 is not None:
        preprocess_from_picam2(picam2, batch_size, input_queue, width, height, preprocess_fn, framerate)
...
def preprocess_from_picam2(picam2,
                           batch_size: int,
    ...
    """
    Process frames from Raspberry Pi camera (picamera2) and enqueue them.
    """
    frames = []
    processed_frames = []
    cam_fps = 30.0  # Default RPi camera FPS
    ...
    try:
        while True:
            ...
            # picamera2 with RGB888 format gives RGB directly
            frames.append(frame)
        ...
        logger.debug(f"Picamera2 capture ended: {e}")
    finally:
        try:
            picam2.stop()
        except:
...
def visualize(
    ...
    if cap is not None:
        ...
        # Handle both cv2.VideoCapture and Picamera2
        if hasattr(cap, 'get'):
            # cv2.VideoCapture
            ...
            # Picamera2 - get resolution from config
            try:
                config = cap.camera_configuration()
                main_config = config.get('main', {})
                size = main_config.get('size', (1280, 720))
                base_width, base_height = size
            except:
                base_width, base_height = 1280, 720
            cam_fps = 30.0  # Picamera2 default
