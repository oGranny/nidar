...
def run_inference_pipeline(net, input, batch_size, labels, output_dir,
    ...
    cap, images, picam2 = init_input_source(input, batch_size, camera_resolution)
    tracker = None
    fps_tracker = None
    if show_fps:
    ...
    preprocess_thread = threading.Thread(
        target=preprocess, args=(images, cap, framerate, batch_size, input_queue, width, height, None, picam2)
    )
    postprocess_thread = threading.Thread(
        target=visualize, args=(output_queue, cap or picam2, save_stream_output,
